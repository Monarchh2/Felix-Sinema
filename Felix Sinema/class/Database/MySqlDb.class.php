...
class MySqlDb
{
     ...
     * @var PDO
     */
    private $conn; // PDO nesnesini temsil eden değişken
    ...
    public function setup($server, $db_name, $username, $password) {
        $this->db = array(
            'server' => $server,
            'db_name' => $db_name,
            'username' => $username,
...
{
    $dsn = 'mysql:host=' . $this->db['server'] . ';dbname=' . $this->db['db_name'] . ';charset=utf8mb4';
    try {
        $pdo = new PDO($dsn, $this->db['username'], $this->db['password']);
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $this->conn = $pdo; // conn özelliğine PDO nesnesini atayın
        return $pdo;
...
{
    $pdo = $this->connect(); // Veritabanına bağlan
